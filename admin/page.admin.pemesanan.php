...
$iduser		= isset($_POST['input_iduser'])?$_POST['input_iduser']:'';
...
if(isset($_POST['input_simpan'])) {
	...
	if($qty >= 64000) {
		$notif = "QTY Tidak Boleh melebihi 64.000";
	} else {
...
if(isset($_POST['input_ubah'])) {
	$id			= getID();
	$str1		= "UPDATE `pemesanan` SET `typeBBM`='$jenisBBM',`qty`='$qty' WHERE `id_pemesanan`='$id'";
	$q1			= $koneksi->query($str1);
	if($q1) {
...
if(isset($_POST['input_delete'])) {
	$id			= getID();
	$str1		= "DELETE FROM `pemesanan` WHERE `id_pemesanan`='$id'";
	$q1			= $koneksi->query($str1);
	if($q1) {
...
$string		= "SELECT a.*,b.`nama_pemilik`,b.`nama_perusahaan`,b.`no_spbu`,b.`lokasi`,c.`gas_type`,c.`price` FROM `pemesanan` AS a LEFT JOIN `customer` AS b ON a.`customer`=b.`id_customer` LEFT JOIN `typebbm` AS c ON a.`typeBBM`=c.`id_typeBBM` where a.status=0 ORDER BY `tanggal`";
...
$strcus		= "SELECT a.*,b.`username`,b.`password` FROM `customer` AS a LEFT JOIN USER AS b ON a.`id_user`=b.`id_user` ORDER BY a.nama_pemilik";
$qcus		= $koneksi->query($strcus);

if($qcus && $qcus->rowCount()>0) {
	$datac	= $qcus->fetchAll(PDO::FETCH_NUM);
} else {
	$datac	= array();
}
                    ...
                    <div class="card-body">
						<form method="post">
							<div class="form-group">
								...
								<select name="input_iduser" class="form-control" onchange="this.options[this.selectedIndex].value && (window.location = '?halaman=pemesanan&aksi=tambah&suser='+this.options[this.selectedIndex].value);">
									<option value="">Pilih Customer</option>
									<?php
									$gses		= isset($_GET['suser'])?$_GET['suser']:'';
									foreach((array)$datac as $dc) {
										$selected	= ($dc[0]==$gses)?'selected':'';
										?><option value="<?php echo $dc[0] ?>" <?php echo $selected ?>>Nama Pemilik : <?php echo $dc[2] ?> ( <?php echo $dc[4] ?> / <?php echo $dc[8] ?> )</option><?php
									}
                ...
                <div class="card">
					...
					if($q1 && $q1->rowCount() > 0) {
						...
						<form method="post">
							<div class="form-group">
								<label>Customer</label>
								<input type="text" class="form-control" disabled="disabled" value="Pemilik : <?php echo $d[6] ?> ( <?php echo $d[7] ?> / <?php echo $d[8] ?> )" />
							</div>
							<div class="form-group">
								<label>Jenis BBM</label>
								<select name="input_jenis" class="form-control">
